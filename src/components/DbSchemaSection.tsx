...
import { Database, Key, ArrowRight, Table2, GitFork, Link2 } from "lucide-react";

export function DbSchemaSection() {
  const [selectedTable, setSelectedTable] = useState<SchemaTable>(DB_TABLES[0]);

  return (
    <section id="database-schema" className="py-20 border-b border-zinc-200 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Visual ER Map */}
        <div className="mb-10 p-6 rounded-lg border border-zinc-300 bg-zinc-50/50 overflow-x-auto">
              ...
              return (
                <div
                  ...
                  className={`cursor-pointer p-3.5 rounded-lg border transition-all flex flex-col justify-between ${
                    isSelected
                      ? "border-blue-600 bg-white ring-2 ring-blue-600 shadow-sm"
                ...
                >
                  ...
                  <div className="mt-3 pt-2 border-t border-zinc-100 text-[9px] font-mono text-zinc-400 truncate">
                ...
                <tbody className="divide-y divide-zinc-100">
                  {selectedTable.fields.map((field) => (
                    <tr key={field.name} className="hover:bg-zinc-50/50">
                      <td className="py-2.5 px-3 font-bold text-zinc-900 flex items-center gap-1.5">
                        {field.name}
                        {field.isPk && (
                          <span className="text-[9px] px-1 py-0.2 rounded bg-zinc-900 text-white">
                            PK
                          </span>
                        )}
                        {field.isFk && (
                          <span className="text-[9px] px-1 py-0.2 rounded bg-blue-100 text-blue-800 border border-blue-200">
                            FK
                          </span>
                        )}
                      </td>
                      ...
                      <td className="py-2.5 px-3 text-zinc-600">{field.description}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="pt-4 border-t border-zinc-200 space-y-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                {selectedTable.relationships.map((rel, idx) => (
                  <div
                    key={idx}
                    className="text-xs font-mono p-2.5 rounded border border-zinc-200 bg-zinc-50 text-zinc-700"
                  >
                    • {rel}
                  </div>
                ))}
