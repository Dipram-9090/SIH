...
export function FutureEnhancementsSection() {
  ...
  return (
    <section id="future-enhancements" className="py-20 border-b border-zinc-200 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {FUTURE_ENHANCEMENTS.map((item) => (
            <div
              ...
              className="cursor-pointer p-6 rounded-lg border border-zinc-300 bg-white shadow-sm hover:border-zinc-500 hover:shadow transition-all flex flex-col justify-between"
            >
              <div>
                <div className="flex items-center justify-between pb-3 mb-3 border-b border-zinc-100">
                  <div className="flex items-center space-x-2.5">
                    <div className="w-8 h-8 rounded border border-zinc-300 bg-zinc-100 flex items-center justify-center text-zinc-900">
                      {icons[item.title] || <Sparkles className="w-4 h-4" />}
                    </div>
                    <span className="text-[10px] font-bold font-mono px-2 py-0.5 rounded border border-zinc-200 bg-zinc-50 text-zinc-700">
                      {item.category}
                    </span>
                  </div>
                </div>
              ...
              <div className="space-y-3 pt-3 border-t border-zinc-100">
                ...
                <div className="flex items-center justify-between text-[10px] font-mono text-zinc-400">
                  ...
                  <span className="text-zinc-700 font-bold hover:underline">
                    View Specs →
                  </span>
                </div>
              </div>
        ...
        {selectedEnhancement && (
          <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-white rounded-lg border border-zinc-300 shadow-2xl max-w-lg w-full overflow-hidden animate-fade-in">
              ...
              <div className="p-6 space-y-4">
                ...
                <div>
                  ...
                  <ul className="space-y-2">
                    {selectedEnhancement.architectureDetails.map((detail, idx) => (
                      <li
                        key={idx}
                        className="text-xs text-zinc-700 p-2.5 rounded border border-zinc-200 bg-zinc-50 flex items-start space-x-2"
                      >
                ...
                <div className="p-3 rounded border border-blue-200 bg-blue-50/50 text-blue-900 text-xs">
              ...
              <div className="p-3 border-t border-zinc-200 bg-zinc-50 flex justify-end">
                <button
                  onClick={() => setSelectedEnhancement(null)}
                  className="px-4 py-1.5 text-xs font-bold rounded bg-zinc-900 text-white"
                >
                  Close
                </button>
              </div>
